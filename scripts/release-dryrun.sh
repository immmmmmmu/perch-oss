...
#   3. For each OSS package, runs `npm publish --dry-run` (no auth, no network)
#      and shows tarball contents via `npm pack --dry-run`
#
# Use:
...
#     v2.27); we validate per-package with `npm publish --dry-run` instead.
#   - Real `changeset version` mutation is verified via the auto-opened
#     "Version Packages" PR after first commit lands on main.
#
...
pnpm -r --filter "@perch/*" build

cyan "==> 2/3: changeset status"
pnpm changeset status

cyan "==> 3/3: npm publish --dry-run for each OSS package"
fail=0
for pkg_dir in "${OSS_PACKAGES[@]}"; do
  cyan "  -- ${pkg_dir} --"
  if ! ( cd "${pkg_dir}" && npm publish --dry-run --access public 2>&1 | grep -E "(npm notice|npm error|name:|version:|filename:|Tarball)" ); then
    red "  FAIL: ${pkg_dir}"
    fail=1
  fi
done
